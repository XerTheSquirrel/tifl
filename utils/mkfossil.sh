...
then
	...
	TARGET="/tmp/tifl.fossil"
fi

# Kill previous fossil repo, if it exists
if [ -f /tmp/tifl-fossil-pid ]
then
	kill $(cat /tmp/tifl-fossil-pid)
	rm -f /tmp/tifl-fossil-pid
fi
...
fossil serve -P $PORT &
FOSSILPID=$!
echo $FOSSILPID > /tmp/tifl-fossil-pid
fossil clone -A tifl "http://127.0.0.1:$PORT" \
	"/tmp/$$.fsl"
kill -- $FOSSILPID
wait
rm -f -- /tmp/tifl-fossil-pid
...
then
	...
	fossil user password tifl "$RPASS" -R "/tmp/$$.fsl"
	fossil user default tifl -R "/tmp/$$.fsl"
	...
	fossil zip "$NOW" "/tmp/$$.zip" --name "tifl" -R "/tmp/$$.fsl"
	...
	then
		...
		fossil tar tip - --name "tifl" -R "/tmp/$$.fsl" \
			| gunzip -c - > "/tmp/$$.tar"
